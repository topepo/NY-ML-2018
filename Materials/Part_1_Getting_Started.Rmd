...
Over this time range, duplicate ratings were eliminated; these occur when the same car is sold for several years in a row. As a result, there are `r nrow(car_train)+nrow(car_test)` cars that are listed in the data. The predictors include the automaker and addition information about the cars (e.g. cylinders, etc). 

In our analysis, the data from 2015-2108 are used for training to see if we can predict the `r nrow(car_test)` cars that were new in 2019. We will restrict ourselves to cars that use some type of gas. 
